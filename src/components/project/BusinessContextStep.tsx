...
const BusinessContextStep: React.FC<BusinessContextStepProps> = ({
  ...
  const handleColumnMappingComplete = (mapping: ColumnMapping) => {
    ...
    setCurrentSubStep(2); // Move to business context after column mapping
  };

  const handleNextStep = () => {
    if (currentSubStep === 1 && hasUploadedData) {
      setCurrentSubStep(2); // Skip to business context if data identification is done
    } else {
      onNext(); // Proceed to next main step
    }
  };
      ...
      return (
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Step 3.1: Understand Your Data</h2>
      ...
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {hasUploadedData ? 'Step 3.2: Business Context' : 'Step 3: Business Context'}
          </h2>
        ...
        <Card className="w-full shadow-sm border-0 bg-white/80 backdrop-blur-sm">
          <CardContent className="p-8">
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={handlePreviousStep} className="flex items-center gap-2 bg-white hover:bg-gray-50">
                <ArrowLeft className="w-4 h-4" />
                {currentSubStep === 2 && hasUploadedData ? 'Back to Data Setup' : 'Previous'}
              </Button>
              <Button 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Summary */}
      {(hasColumnMapping || additionalContext.trim()) && currentSubStep === 2 && (
