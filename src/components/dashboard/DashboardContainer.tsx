...
const DashboardContainer: React.FC<DashboardContainerProps> = ({ data }) => {
  ...
  const normalizedData: ParsedData = {
    ...
    rowCount: processedData && 'rowCount' in processedData ? processedData.rowCount : processedData?.rows?.length || 0,
    fileSize: processedData && 'fileSize' in processedData ? processedData.fileSize : 0,
